...
def _shell():
    ...
    shell, _ = create_local_message(shell, receiver_hint="Auri", body="Receipt-chain test")
    return shell


def test_persist_and_restore_receipts_form_verified_chain(tmp_path):
    ...
    assert [receipt["event"] for receipt in chain] == ["persist", "restore"]
    ...
    assert verification["authorizing"] is False


def test_authority_rejection_is_recorded_without_manual_chain_work(tmp_path):
    ...
    assert chain[0]["result"] == "rejected"
    assert "authority boundary violated" in chain[0]["reason"]
    assert verify_session_receipt_chain(chain)["verified"] is True


def test_integrity_failure_is_chained_automatically(tmp_path):
    _, chain = persist_session_with_receipt(
        store_root=tmp_path,
        shell=_shell(),
        session_id="primary",
    )
    path = tmp_path / "mobile_shell_sessions" / "primary.json"
    path.write_text(path.read_text(encoding="utf-8").replace("Receipt-chain test", "Tampered"), encoding="utf-8")
    ...
    assert chain[-1]["previous_chain_head"] == chain[-2]["receipt_chain_head"]
    assert verify_session_receipt_chain(chain)["verified"] is True


def test_receipt_replay_detects_discontinuity():
...
def test_receipt_summary_is_payload_free_and_non_authorizing(tmp_path):
    _, chain = persist_session_with_receipt(
        store_root=tmp_path,
        shell=_shell(),
        session_id="primary",
    ...
    assert summary["events"] == ["persist"]
    assert summary["session_ids"] == ["primary"]

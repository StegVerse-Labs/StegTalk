...
def test_queue_summary_reports_current_managed_queue_without_freezing_task_count():
    queue = load_queue(ROOT)
    summary = summarize_queue(queue)
    assert summary["task_count"] == len(queue["tasks"])
    assert summary["production_ready"] is False
    assert summary["next_task"] is None
    assert summary["complete_count"] == sum(1 for task in queue["tasks"] if task.get("status") == "complete")
...
def test_queue_execution_verifies_current_queue_shape():
    queue = load_queue(ROOT)

...
from .mobile_shell_session import (
...
def create_session_event_receipt(
    *,
    session_id: str,
    event: str,
    result: str,
...
def verify_session_receipt_chain(chain: list[JsonObject]) -> JsonObject:
    ...
    for index, receipt in enumerate(chain):
        ...
        expected_id = with_receipt_identity(rebuilt, previous_chain_head=previous)["id"]
        expected_head = stable_hash({"previous": previous, "receipt": {**rebuilt, "id": expected_id}})
        if receipt.get("id") != expected_id:
            raise ValueError(f"receipt identity mismatch at index {index}")
        if receipt.get("receipt_chain_head") != expected_head:
...
def persist_session_with_receipt(
    *,
    store_root: str | Path,
    shell: JsonObject,
    session_id: str,
    chain: list[JsonObject] | None = None,
    ...
    try:
        ...
        receipt = create_session_event_receipt(
            session_id=session_id,
            event="rejection",
            result="rejected",
...
def restore_session_with_receipt(
    *,
    store_root: str | Path,
    session_id: str,
    chain: list[JsonObject] | None = None,
    ...
    receipt = create_session_event_receipt(
        ...
        result="success",
        previous_chain_head=previous,
        snapshot_hash=stable_hash({key: value for key, value in shell.items() if key != "mobile_shell_hash"}),
    )
...
def summarize_session_receipts(chain: list[JsonObject]) -> JsonObject:
    ...
    return {
        **verification,
        ...
        "results": [receipt["result"] for receipt in chain],
        "session_ids": sorted({receipt["session_id"] for receipt in chain}),
    }
...
def _assert_receipt_boundary(receipt: JsonObject) -> None:
    ...
    if receipt.get("production_ready") is not False:
        raise ValueError("session receipt must remain non-production")
    if receipt.get("local_only") is not True:
        raise ValueError("session receipt must remain local-only")
    if receipt.get("authorizing") is not False:
        raise ValueError("session receipt must remain non-authorizing")

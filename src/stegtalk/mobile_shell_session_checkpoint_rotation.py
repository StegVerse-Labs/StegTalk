...
def rotate_managed_checkpoint(*, store_root: str | Path, shell: JsonObject, session_id: str, retention: int = DEFAULT_RETENTION) -> JsonObject:
    ...
    return {"session_id": session_id, "generation": generation, "retained_generations": [item["generation"] for item in updated], "current_checkpoint_hash": created["checkpoint_hash"], "history_hash": record["history_hash"], "manual_tasks_required": [], "production_ready": False, "local_only": True, "authorizing": False}
...
def restore_latest_valid_checkpoint(*, store_root: str | Path, session_id: str) -> JsonObject:
    ...
    for entry in reversed(record["entries"]):
        try:
            ...
            continue
        return {"session_id": session_id, "generation": entry["generation"], "shell": entry["shell"], "receipt_chain": entry["receipt_chain"], "checkpoint_hash": entry["checkpoint_hash"], "fallback_used": entry["generation"] != record["entries"][-1]["generation"], "rejected_generations": failures, "verified": True, "production_ready": False, "local_only": True, "authorizing": False}
...
def inspect_checkpoint_history(*, store_root: str | Path, session_id: str) -> JsonObject:
    record = _read_history_record(store_root, session_id)
    return {"session_id": session_id, "retention": record["retention"], "retained_generations": [entry["generation"] for entry in record["entries"]], "history_hash": record["history_hash"], "production_ready": False, "local_only": True, "authorizing": False}


def _build_history_record(*, session_id: str, retention: int, entries: list[JsonObject]) -> JsonObject:
    record = {"schema_version": "1.0.0", "record_type": "stegtalk_mobile_shell_session_checkpoint_history", "session_id": session_id, "retention": retention, "entries": entries, "entry_count": len(entries), "production_ready": False, "local_only": True, "authorizing": False, "updated_at": utc_now()}
    record["history_hash"] = stable_hash(record)
    return record


def _read_history_record(store_root: str | Path, session_id: str) -> JsonObject:
    record = json.loads(_history_path(store_root, session_id).read_text(encoding="utf-8"))
